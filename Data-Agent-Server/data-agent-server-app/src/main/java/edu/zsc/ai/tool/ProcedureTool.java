...
public class ProcedureTool {
    ...
    @Tool({
        "Get the list of all stored procedure names and metadata in the current database/schema.",
        "Use when the user asks what stored procedures exist or to explore database procedures. Pass connectionId, databaseName, schemaName from current session context."
    })
    public String getProcedureNames(
        ...
        try {
            ...
            List<ProcedureMetadata> procedures = procedureService.listProcedures(
                    connectionId,
                    databaseName,
                    schemaName,
                    userId
    ...
    public String getProcedureDdl(
            @P("The exact name of the stored procedure in the current schema") String procedureName,
            @P("Connection id from current session context") Long connectionId,
            @P("Database (catalog) name from current session context") String databaseName,
            @P(value = "Schema name from current session context; omit if not used", required = false) String schemaName,
            InvocationParameters parameters) {
        ...
        try {
            ...
            log.info("{} getProcedureDdl, procedureName={}, ddlLength={}", ToolMessageConstants.TOOL_LOG_PREFIX_DONE,
                    procedureName, ddl != null ? ddl.length() : 0);
            return ddl;
        } catch (Exception e) {
