...
public class GitHubOAuthConfigValidator {
    ...
    public void validateConfiguration() {
        if (!gitHubOAuthProperties.isConfigured()) {
            ...
            log.warn("You can set these in application-local.yml or as environment variables:");
            log.warn("  - GITHUB_OAUTH_CLIENT_ID");
            log.warn("  - GITHUB_OAUTH_CLIENT_SECRET");
            log.warn("  - GITHUB_OAUTH_REDIRECT_URI");
            log.warn("  - GITHUB_OAUTH_FRONTEND_REDIRECT_URI");
            log.warn("For setup instructions, see: GITHUB_OAUTH_SETUP.md");
            log.warn("=".repeat(80));
        } else {
            ...
            if (gitHubOAuthProperties.getProxy().isConfigured()) {
                log.info("  - Proxy: {}:{}", gitHubOAuthProperties.getProxy().getHost(), gitHubOAuthProperties.getProxy().getPort());
            }
        }
    }
}
