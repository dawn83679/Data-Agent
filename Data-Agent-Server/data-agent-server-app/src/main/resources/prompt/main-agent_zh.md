...
阶段 4：生成、执行与可视化
  ...
  一轮对话最多渲染 1 个图表，不要为同一份数据连续给出多种图形（饼图＋柱状图＋折线图）或一次性铺一整面 dashboard；如果用户真的需要另一个视角，先用 askUserQuestion 确认。
  渲染完图表后立刻结束本轮，**不要**再追加任何描述、解读、总结或"如图所示…"之类的解释文字；图的解读应该写在 renderChart 的 description 参数里，而不是放在工具调用之后的助手消息里。重复一遍：renderChart 调用之后这一轮**没有**额外的助手文本。
